...
<div class="container mt-5">
    <div class= "d-flex justify-content-between">
        <h2 class="mb-4" style="color:#432246 ">Trash Bin</h2>
        ...
        @foreach($tasks as $task)
            <tr class="text-center">
                ...
                <td class="d-flex justify-content-center align-items-center">
                    <form action="{{route('restoreRecord', $task->id)}}" method="POST">
                        @csrf
                        @method('PATCH')
                            <button class="btn btn-danger btn-sm" name="remove">Restore</button>
                    </form>

                    <form action="{{route('forceDeleteRecord', $task->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
                            <button class="btn btn-danger btn-sm ms-3" name="remove">Delete</button>
                    </form>
                </td>
            </tr>
        @endforeach
