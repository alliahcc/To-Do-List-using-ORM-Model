...
<div class="container mt-5">
    <div class= "d-flex justify-content-between">
        <h2 class="mb-4" style="color:#432246 ">In Progress</h2>
            ...
            <tr class="text-center">
               ...
               <td class="text-center">
                ...
                @if($task->status == 'In Progress')
                    <form action="{{route('completeTask', $task->id)}}" method="POST">
                        @csrf
                        @method('PATCH')
                            <button type="submit" class="btn btn-danger btn-sm" name="remove">Done</button>
                    </form>
                @endif
                </td>
                <td class="d-flex justify-content-center align-items-center">
                    <!-- show edit form -->
                    <a href="{{ route('editRecord', $task->id) }}" class="btn btn-secondary btn-sm">Edit</a> 

                    <form action="{{ route('softDeleteRecord', $task->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                            <button class="btn btn-danger btn-sm ms-3" name="remove">Remove</button>
                    </form>
                </td>
            </tr>
        @endforeach
